...
private fun solution2(board: Array<IntArray>): Int {
board.indices.map { row ->
    board[row].indices.filter { board[row][it] == 1}.forEach {col ->
        ((row - 1).coerceAtLeast(0)..(row + 1).coerceAtMost(board.lastIndex)).map { println(it)}
    }
}
...
fun main() {
    ...
    var testC = arrayOf(intArrayOf(0,1,0), intArrayOf(0,1,0), intArrayOf(1,1,1))
